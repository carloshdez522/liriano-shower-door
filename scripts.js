(function () {
  ...
  var translations = {
    en: {
      ...
      service2_desc: 'Custom glass cabinets for closets, display cases, or storefronts. Sliding or hinged doors, clear or frosted glass. Built to your measurements and style.',
      service3_title: 'Partitions & Railings',
      service3_desc: 'Railings, balustrades, and glass partitions for indoors and outdoors. Safety without sacrificing the view, with designs ranging from minimalist handrails to pool fencing and glass balcony enclosures.',
      service4_title: 'Repair & Replacement',
      service4_desc: 'Repair and replacement of all types of glass: windows, shower doors, railings, mirrors, and more. Fast, professional service with the same quality standards as a new installation.',
      service5_title: 'Custom Mirrors & Glass',
      service5_desc: 'Bespoke mirrors, etched glass designs, and custom-cut glass solutions for residential and commercial spaces.',
    ...
    es: {
      ...
      service2_desc: 'Gabinetes de vidrio a medida para closets, vitrinas o expositores. Puertas corredizas o abatibles, vidrios claros, esmerilados o con dise\u00f1o. Fabricados seg\u00fan tus medidas y estilo.',
      service3_title: 'Particiones y Barandales',
      service3_desc: 'Barandales, balaustradas y particiones de vidrio para interiores y exteriores. Seguridad sin perder la vista, con dise\u00f1os que van desde pasamanos minimalistas hasta cercas para piscina y balcones de cristal.',
      service4_title: 'Reparaci\u00f3n y Reemplazo',
      service4_desc: 'Reparaci\u00f3n y reemplazo de todo tipo de vidrio: ventanas, puertas de ducha, barandales, espejos y m\u00e1s. Servicio r\u00e1pido, profesional y con los mismos est\u00e1ndares de calidad que en una instalaci\u00f3n nueva.',
      service5_title: 'Espejos y Vidrios Personalizados',
      service5_desc: 'Espejos a medida, dise\u00f1os de vidrio grabado y soluciones de vidrio cortado a la medida para espacios residenciales y comerciales.',
  ...
  function getStep() { var c = track.querySelector('.carousel-card'); return c ? c.offsetWidth + 24 : 364; }
  ...
  function goTo(index, instant) {
    ...
    track.style.transform = 'translateX(' + (-current * s) + 'px)';
  ...
  function onDragEnd() { if (!isDragging) return; isDragging = false; track.classList.remove('dragging'); var s = window.getComputedStyle(track).transform; var fx = 0; if (s !== 'none') { var v = s.match(/-?\d+\.?\d*/g); if (v) fx = parseFloat(v[4]); } var st = getStep(); var diff = fx - (-current * st); track.style.transition = 'transform 0.4s ease'; if (diff < -60) { next(); } else if (diff > 60) { prev(); } else { track.style.transform = 'translateX(' + (-current * st) + 'px)'; } setTimeout(function() { isAnimating = false; }, 450); }
